...
//		session.removeAttribute("acc");
		...
		String confirm = request.getParameter("exampleRepeatPassword");
		...
		if (users.getPassword().equals(confirm)) {
			...
			try {
				homeServer.addNewUser(users);
				session1.setAttribute("acc", users);
				modelAndView.setViewName("customer/login");
			} catch (Exception e) {
...
//					message.setText(emailContent01);
	...
	public ModelAndView loginbyFb(HttpServletResponse response, HttpServletRequest request) throws IOException {
		...
		if (users == null) {
			...
			session.setAttribute("acc", homeServer.timKiemUser(pass));
			response.sendRedirect("home");
		}
		return modelAndView;
	}
