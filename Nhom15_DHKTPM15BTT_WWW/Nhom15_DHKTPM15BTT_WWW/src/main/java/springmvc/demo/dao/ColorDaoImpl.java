...
//		query.setParameter("index", (6*index)-5);
	...
	public List<Object[]> dsColorTop6(int index, String string) {
		String hql = "select c.product.productId ,c.img, c.product.name, c.product.price  from Color c where c.product.name like '%"
				+ string + "%' group by c.product.productId,c.img, c.product.name, c.product.price\r\n"
				+ "		order by c.product.productId\r\n";
		Session currentSession = sessionFactory.getCurrentSession();
		TypedQuery<Object[]> query = currentSession.createQuery(hql, Object[].class)
				.setHibernateFirstResult(((index - 1) * 6)).setMaxResults(6);
...
//	GROUP BY Color.name, Color.code
	...
	public void deleteColor(String id) {
		Session currentSession = sessionFactory.getCurrentSession();
		Query<Color> query = currentSession.createQuery("delete from Color where product_id=:product_id");
	...
	public int demSLColorTheoProductId(String productId) {
		try {
			String query = "select count(product_id) from Color\r\n" + "where [product_id]='" + productId + "'";
			Session currentSession = sessionFactory.getCurrentSession();
	...
	public Color getColor(String theId) {
		Session currentSession = sessionFactory.getCurrentSession();
		Color theColor = currentSession.get(Color.class, theId);
		return theColor;
	}
	@Override
	public void deleteColorById(String theId) {
        ...
        Color color = session.byId(Color.class).load(theId);
